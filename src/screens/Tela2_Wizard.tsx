...
const Tela2_Wizard: React.FC<Tela2Props> = ({ data, onChange, onFinish, onBack }) => {
  ...
  const renderForm = () => {
    switch (step) {
      case 0: // Capa
        return (
          <div className="space-y-4">
            <ToggleSection label="Exibir logo BM" enabled={data.showLogo} onToggle={(v) => update({ showLogo: v })}>
              <div />
            </ToggleSection>
            <div>
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              <button onClick={() => fileInputRef.current?.click()} className="bg-bm-card border border-bm-orange/30 rounded-lg p-2 text-sm text-bm-gray hover:border-bm-orange w-full text-left">
      ...
      case 2: // Sobre a BM
        return (
          <div className="text-sm text-bm-gray">
            <p className="bg-bm-card rounded-lg p-3 border border-bm-orange/20 text-bm-gray-dark">
              ⚠️ Conteúdo padrão BM — não editável. Use o toggle para incluir ou excluir este slide.
            </p>
          </div>
        );
      case 3: // Divisão de responsabilidades
        return (
          <div className="space-y-3">
            ...
            <div>
              ...
              {data.responsabilidadesEspecialista.map((r, i) => (
                <input key={i} value={r} onChange={(e) => {
                  const arr = [...data.responsabilidadesEspecialista];
            ...
            <div>
              ...
              {data.responsabilidadesBM.map((r, i) => (
                <input key={i} value={r} onChange={(e) => {
                  const arr = [...data.responsabilidadesBM];
                  arr[i] = e.target.value;
      ...
      case 4: // Estrutura completa
        return (
          <p className="text-sm text-bm-gray bg-bm-card rounded-lg p-3 border border-bm-orange/20">
            ⚠️ Conteúdo padrão BM — 8 áreas de operação. Use o toggle para incluir ou excluir.
          </p>
        );
      case 5: // Funil 8 overview
        return (
          <div className="space-y-3">
            <p className="text-xs text-bm-gray">Visão geral das 3 fases do Funil 8. Opcional — nem todos os especialistas precisam desta metodologia.</p>
      ...
      case 6: // Funil 8 Fase 1
        return <p className="text-xs text-bm-gray">Detalhamento da pirâmide de aquisição. Valores editáveis no preview.</p>;
      case 7: // Fases 2 e 3
            ...
            {data.proximosPassos.map((p, i) => (
              <div key={i} className="space-y-1">
                ...
                <input value={p.titulo} onChange={(e) => {
                  const arr = [...data.proximosPassos];
                  arr[i] = { ...arr[i], titulo: e.target.value };
                  update({ proximosPassos: arr });
                }} className="w-full bg-bm-card border border-bm-orange/20 rounded p-1.5 text-xs text-bm-white focus:outline-none" placeholder="Título" />
                <input value={p.dias} onChange={(e) => {
                  const arr = [...data.proximosPassos];
                  arr[i] = { ...arr[i], dias: e.target.value };
                  update({ proximosPassos: arr });
                }} className="w-full bg-bm-card border border-bm-orange/20 rounded p-1.5 text-xs text-bm-white focus:outline-none" placeholder="Range de dias" />
                <textarea value={p.descricao} onChange={(e) => {
                  const arr = [...data.proximosPassos];
                  arr[i] = { ...arr[i], descricao: e.target.value };
                  update({ proximosPassos: arr });
                }} rows={2} className="w-full bg-bm-card border border-bm-orange/20 rounded p-1.5 text-xs text-bm-white focus:outline-none resize-none" placeholder="Descrição" />
              </div>
            ))}
          </div>
        );
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full p-4 flex flex-col lg:flex-row gap-4">
        ...
        <div className="lg:w-[45%] space-y-4 overflow-y-auto">
          <div className="bg-bm-card rounded-xl p-4 border border-bm-orange/20">
            ...
            <ToggleSection
              label={`Incluir este slide`}
              tooltip={config.tooltip}
              enabled={data[toggleKey] as boolean}
              onToggle={(v) => update({ [toggleKey]: v })}
            >
              {renderForm()}
            </ToggleSection>
          </div>
        ...
        <div className="lg:w-[55%] flex items-start justify-center">
          <div className="w-full min-w-[560px] rounded-xl overflow-hidden shadow-2xl border border-bm-orange/20">
            {renderPreview()}
          </div>
        </div>
      </div>
